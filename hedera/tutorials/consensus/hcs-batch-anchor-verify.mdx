...
<Accordion title="View the code">
   ...
   * @returns {Promise<{topicId: string, transactionId: string}>}
   */
...
Now run the main script. This script orchestrates the following process:
...
5.  **Build Message**: Creates a JSON object containing the root and metadata.
6.  **Submit**: Sends the JSON string to the HCS topic using `submitMessage`.
...
<Accordion title="View the code">
  <CodeGroup>
  ```javascript scripts/02-anchor-batch.js
  const fs = require('fs');
...
<Accordion title="View the code">
  The `03-verify-batch.js` script confirms the batch integrity:
  1.  **Recompute Root**: Loads the local dataset and calculates the Merkle root exactly as before using `computeRoot`.
  2.  **Fetch Message**: Queries the Mirror Node API for the latest message on the topic using `getLatestTopicMessage`.
  3.  **Compare**: decoding the message and verifying that the on-chain root matches the locally computed root.


  <CodeGroup>
  ```javascript scripts/03-verify-batch.js
  const fs = require('fs');
  const path = require('path');
  const { canonicalize } = require('../src/canonicalize');
  const { sha256 } = require('../src/hash');
...
<Accordion title="View the code">
  The `04-verify-single-record.js` script demonstrates Merkle proofs:
  1.  **Load Proof**: Reads a pre-generated Merkle proof for the specific record.
  2.  **Trusted Root**: In a real scenario, this comes from HCS (as in step 5). Here we simulate it with a manifest.
  3.  **Verify**: Uses `verifyProof` to hash the record with its sibling hashes up the tree. If the final hash matches the trusted root, the record is proven.


  <CodeGroup>
  ```javascript scripts/04-verify-single-record.js
  const fs = require('fs');
  const path = require('path');
...
async function createTopic() {
  ...
  const tx = await new TopicCreateTransaction()
    .setTopicMemo("HCS Batch, Anchor, and Verify Tutorial")
    .execute(client);
